...
def main(argv):    
    ...
    l_ind = argv.index('-l')
    u_ind = argv.index('-u')
    int_ind = argv.index('-i')
    n_ind = argv.index('-n')
    


    num_cores = int(argv[n_ind+1])


    POS_X_UB = Decimal(argv[u_ind+1])
    POS_X_LB = Decimal(argv[l_ind+1])
    curUBPos = POS_X_UB
    posOffset = Decimal(argv[int_ind+1])
    numRays = 21
    dnnYamls = []
    noiser_size = ''
    ...
    modelFile = modelFolder + '/testModel' + str(posOffset)
    ...
    dnnYamls = ['noiser5x100.yml', 'denoiser.yml', 'controller.yml']
    noiser_size = '5x100'
    dnns = []
    ...
    num_x_instances = round((POS_X_UB - POS_X_LB) / posOffset)
    ...
    while x_ind < num_x_instances:
        initProps = ['y1 in [' + str(curUBPos-posOffset) + ', ' + str(curUBPos) + ']',
                     'y2 in [9, 9]',
                     'y3 in [0, 0]',
                     'y4 in [0, 0]', 'k in [0, 0]',
                     'u in [0, 0]', 'angle in [0, 0]', 'temp1 in [0, 0]', 'temp2 in [0, 0]',
                     'theta_l in [0, 0]', 'theta_r in [0, 0]']  # F1/10  # F1/10

        curModelFile = modelFile +'right_'+str(curUBPos)+'_' + str(count) + '.model'
        writeComposedSystem(curModelFile, initProps, dnns, plant, glue, safetyProps, NUM_STEPS, noiser_size)

        dnn_string = ''
        for dnn_name in dnnYamls:
            dnn_string += dnn_name + ' '

        #dnn_string = big + ' ' + straight_little + ' ' + square_right_little + ' ' + square_left_little + \
        #             ' ' + sharp_right_little + ' ' + sharp_left_little + ' ' + square_controller + ' ' + sharp_controller

        args = '../flowstar/flowstar ' + dnn_string + '< ' + curModelFile
    ...
    while cur_process <= num_cores:
        ...
        p = Popen(all_args[cur_process], shell=True, stdin=PIPE)
